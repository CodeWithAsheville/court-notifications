...
async function getConfigurationIntValue(name, defaultValue = 0) {
	...
	let result = await knex('cn_configuration')
		.select('value')
		.where('name', '=', name);
...
async function purgeSubscriptions() {
	const daysBeforePurge = await getConfigurationIntValue(
		'days_before_purge',
		30,
	);
	...
	let count = await knex('defendants')
		.delete()
		.where('last_valid_cases_date', '<', purgeDate);

	if (count > 0) {
		logger.debug(count + ' defendants purged');
		count = await knex('subscriptions')
			.delete()
			.whereNotExists(function () {
				this.select('*')
					.from('defendants')
					.whereRaw('defendants.id = subscriptions.defendant_id');
			});
		...
		subscribers = await knex('subscribers')
			.select(
				...
				knex.raw(
					'PGP_SYM_DECRYPT("subscribers"."encrypted_phone"::bytea, ?) as phone',
			...
			.whereNotExists(function () {
				this.select('*')
					.from('subscriptions')
					.whereRaw('subscriptions.subscriber_id = subscribers.id');
			});
...
async function updateSubscriptions() {
	const daysBeforeUpdate = await getConfigurationIntValue(
		'days_before_update',
		7,
	);
	...
	const defendantsToUpdate = await knex('defendants')
		.select('id as defendant_id')
