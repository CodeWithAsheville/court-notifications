...
async function updateDefendants(purgeDate, updateDays) {
	...
	const config = await knex('cn_configuration')
		.select('value')
		.where('name', '=', 'updates_per_call');
	if (config.length > 0) {
		updatesPerCall = parseInt(config[0].value);
	}
	// Get a set of defendants to update
	const defendantsToUpdate = await knex('records_to_update')
		.select('defendant_id')
		.where('done', '=', 0)
		.limit(updatesPerCall);
	...
	await knex('records_to_update')
		.delete()
		.where('defendant_id', 'in', defendantIds);
	const defendants = await knex('defendants')
		.select('*')
		.where('id', 'in', defendantIds);

	const dt = new Date();
	const updateObject = {
		last_valid_cases_date:
	...
	for (let i = 0; i < defendants.length; ++i) {
		...
		await knex('defendants').where('id', '=', d.id).update(updateObject);
	}
}
