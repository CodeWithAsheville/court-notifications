...
async function loadDefendants(notificationDays) {
	...
	const results = await knex('cases')
		.select(
			...
			'defendants.middle_name',
			'defendants.last_name',
			'defendants.suffix',
			'defendants.birth_date',
		)
...
async function sendNotifications() {
	...
	for (i = 0; i < notificationSets.length; ++i) {
		logger.debug(
			'Do notifications for ' + notificationSets[i].days_before + ' days',
		);
			...
			for (k = 0; k < subscribers.length; ++k) {
				...
				if (defendant.adminCount > 0) {
					message += Mustache.render(
						i18next.t('notifications.admin-court'),
						defendant,
					);
				}
				if (defendant.districtCount > 0) {
					message += Mustache.render(
						i18next.t('notifications.district-court'),
						defendant,
					);
					let sep = '';
				...
				if (defendant.superiorCount > 0) {
					message += Mustache.render(
						i18next.t('notifications.superior-court'),
						defendant,
					);
				}
				...
				await client.messages
					.create(msgObject)
					.then((sentMessage) =>
						logger.debug(JSON.stringify(sentMessage.body)),
					);
			}
		}
	}
